...
### Específico do Paint-V2

O núcleo (`src/paintv2/core/`) é **puro NumPy, sem importar Qt**. É o que torna o motor
de pintura testável sem abrir janela — mantenha assim. Qt entra a partir de
`src/paintv2/ui/` e de `tools/` (que traduz eventos de mouse em chamadas do núcleo).

Um pincel é a combinação de uma **ponta** (`core/brush_tips.py` — forma e dinâmica do
carimbo) com um **modo** (`core/brush_modes.py` — o que acontece com os pixels sob a
máscara). Ferramenta nova de efeito = um modo novo; nunca duplique pontas.
